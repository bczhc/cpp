...
class SnakeGame {
	...
	void start() {
		while (!gameover) {
			...
			snake.putFirst(next);
			if (c != '$') {Point last = snake.removeLast(); MAP(last.x, last.y) = ' ';}
			c = MAP(next.x, next.y);
			if (c == '*' || c == 'X') {gameover = 1; goto a;}
			if (c == '$') generateFood();
			drawToMap();
			print();
			usleep(250000);
a:;
	...
	void moveUp() {orientation = UP;}

	void moveDown() {orientation = DOWN;}

	void moveLeft() {orientation = LEFT;}

	void moveRight() {orientation = RIGHT;}
};
