...
char *bczhc::ToUpperCase(char *Dest, const char *string) {
	char *p = Dest;
...
void bczhc::PrintArr(const char arr[], int len) {
	int l_ = len - 1;
	printf("[");
	for (int i = 0; i < l_; ++i) {
...
int bczhc::BinToDec(const char *NumStr) {
	int r = 0;
	int j = 0;
...
void bczhc::printArr(const char *a, const int length) {
	int l = length;
	printf("[");
	for (int i = 0; i < l; ++i) {
		printf("%i", (int)a[i]);
...
void bczhc::substr(char **Dest, const char *source, const int from, int length) {
	*Dest = (char *)malloc((size_t)length + 1);
	strncpy(*Dest, source + from, (size_t)length);
}

char *bczhc::substr2(char *Dest, const char *source, const int from, int length) {
	char *r = Dest;
...
int bczhc::getIntegerLen(const int x) {
	int n = x;
	int r = 0;
	while (1) {
		int b = n / 10;
...
int bczhc::getLongLen(const long x) {
	long n = x;
	int r = 0;
	while (1) {
		long b = n / 10;
 ...
 */
uint32_t bczhc::strInStrCount(int **Dest, const char *string, const char *s) {
	uint32_t c = 0;
...
int bczhc::Str_Cmp_nMatchCase(const char *a, const char *b) {
	char t1[strlen(a) + 1];
	char t2[strlen(a) + 1];
	ToUpperCase(t1, a);
	ToUpperCase(t2, b);
...
int bczhc::split(char ***Dest, const char *SourceString, const char *SplitStr) {
	int *pos = NULL;
...
int bczhc::cmpIntArray(const int *a1, const int a1Len, const int *a2,
		const int a2Len) {
	if (a1Len != a2Len)
		return 0;
	else {
...
int bczhc::cmpCharArray(const char *a1, const int a1Len, const char *a2,
		const int a2Len) {
	if (a1Len != a2Len)
		return 0;
	else {
...
int bczhc::getBiggerNum(const int a, const int b) { return a > b ? a : b; }

int bczhc::firstIndexOf(const char *s, const int s_len, const char c) {
...
int bczhc::Split(char ***dst, const char *s, int s_length, const char *separatorString,
...
bczhc::String::String(const char *s) : String(s, strlen(s)) {}

bczhc::String::String(const char *s, size_t length) {
	this->init(s, length);
}
...
char *bczhc::String::getData() { return data; }

bczhc::String String::append(const char *s) {
...
bczhc::String String::append(String &s) {
	char *sData = s.getData();
	return this->append(sData);
}

bczhc::ArrayList<String *> String::split(const char *separator) {
	char **r = nullptr;
...
bczhc::ArrayList<String *> String::split(String &separator) {
	char *separatorData = separator.getData();
	return this->split(separatorData);
}


string bczhc::String::toCppString() {
	string r(this->getData());
	return r;
}

String &bczhc::String::operator=(const char *s) {
...
int bczhc::utf8::getUTF8BytesLength(const char firstByte) {
 ...
 void bczhc::utf8::solveUTF8Bytes(utf8::SolvedUTF8Properties &solvedProperties, const char *bytes) {
	...
	switch (solvedProperties.bytesLength) {
		...
		case 5:
			solvedProperties.codepoint =
				(bytes[4] & 0b00111111) | ((bytes[3] & 0b00111111) << 6) |
				((bytes[2] & 0b00001111) << 12) | ((bytes[1] & 0b00000111) << 18) |
				((bytes[0] & 0b00000011) << 24);
			break;
		case 6:
			solvedProperties.codepoint =
				(bytes[5] & 0b00111111) | ((bytes[4] & 0b00111111) << 6) |
				((bytes[3] & 0b00001111) << 12) | ((bytes[2] & 0b00000111) << 18) |
				((bytes[1] & 0b00000011) << 24) | ((bytes[0] & 0b00000001) << 30);
			break;
	}
}
